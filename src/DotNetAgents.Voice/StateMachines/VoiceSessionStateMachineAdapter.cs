...
/// <summary>
/// Adapter that wraps a state machine instance to implement IVoiceSessionStateMachine.
/// Uses reflection to avoid direct dependency on IStateMachine.
/// </summary>
/// <typeparam name="TState">The type of the state context.</typeparam>
public class VoiceSessionStateMachineAdapter<TState> : IVoiceSessionStateMachine<TState> where TState : class
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="VoiceSessionStateMachineAdapter{TState}"/> class.
    /// </summary>
    /// <param name="stateMachine">The state machine instance to wrap.</param>
    public VoiceSessionStateMachineAdapter(object stateMachine)
    {
