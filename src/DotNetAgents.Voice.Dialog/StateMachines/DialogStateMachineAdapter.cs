...
/// <summary>
/// Adapter that wraps a state machine instance to implement IDialogStateMachine.
/// Uses reflection to avoid direct dependency on IStateMachine.
/// </summary>
/// <typeparam name="TState">The type of the state context.</typeparam>
public class DialogStateMachineAdapter<TState> : IDialogStateMachine<TState> where TState : class
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="DialogStateMachineAdapter{TState}"/> class.
    /// </summary>
    /// <param name="stateMachine">The state machine instance to wrap.</param>
    public DialogStateMachineAdapter(object stateMachine)
    {
