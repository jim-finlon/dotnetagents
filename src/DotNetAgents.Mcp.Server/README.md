...
- **POST /mcp** — JSON-RPC (same `IMcpToolProvider`). REST-style clients can continue to use `/mcp/tools` and `/mcp/tools/call`. Endpoints use **`DisableAntiforgery()`** so Blazor hosts with `UseAntiforgery()` still accept remote MCP clients' JSON `POST /mcp`.
- **GET /mcp** — returns **405** (no server SSE stream in this minimal implementation).

**Examples:** a document tool service, a project-management service, an
education tutor service, a media workflow service, or a repository intelligence
service can all expose the same MCP shape. Point your MCP client `"url"` at
`http://<host>:<port>/mcp` when deployed.
...
See [MCP-CONTRACT-COMPATIBILITY](../../docs/guides/MCP-CONTRACT-COMPATIBILITY.md).
